...
def focal_image_url(request):
    return request.config.getoption("--image-cache-url") or \
...
def get_vm_agent_connected_cnt(api_client, vm_names):
    cnt = 0
    for vm_name in vm_names:
        _, data = api_client.vms.get(vm_name)
        if data.get('status', {}).get('ready', False):
            _, data = api_client.vms.get_status(vm_name)
...
def test_multiple_migrations(api_client, unique_name, focal_image, wait_timeout,
    ...
    for vm_name in vm_names:
        code, data = api_client.vms.get_status(vm_name)
        assert 200 == code, (
            f"Failed to get VM {vm_name} with error: {code}, {data}"
        )
    ...
    for vm_name, src_dst_hosts in vm_src_dst_hosts.items():
        endtime = datetime.now() + timedelta(seconds=wait_timeout)
        code, data = None, None
        while endtime > datetime.now():
            code, data = api_client.vms.get_status(vm_name)
            if data.get('status', {}).get('migrationState', {}).get('completed', False):
                assert src_dst_hosts[1] == data['status']['nodeName'], (
                    f"Failed to migrate VM {vm_name} \
                        from {src_dst_hosts[0]} to {src_dst_hosts[1]}"
                )
                break
            sleep(5)
        else:
...
def test_migrate_vm_with_user_data(api_client, unique_name, focal_image, wait_timeout,
    ...
    code, vmi_data = api_client.vms.get_status(unique_name)
    assert 200 == code, (
        f"Failed to get VM {unique_name} with error: {code}, {vmi_data}"
    )
    src_host = vmi_data['status']['nodeName']
    dst_host = ""
    for node_name in available_node_names:
        if node_name != src_host:
            dst_host = node_name
